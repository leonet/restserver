...
class Restserver
{
	...
	{
        $request = $this->_get_method();
        $method  = (!empty($request)) ? strtolower($request) : '';
	...
	{
        return $this->_required_method($str, 'post');
	}
    
    public function required_get($str)
	{
        return $this->_required_method($str, 'get');
	}
    
    public function required_put($str)
	{
        return $this->_required_method($str, 'put');
	}
    
    public function required_patch($str)
	{
        return $this->_required_method($str, 'patch');
	}
    
    public function required_delete($str)
	{
        return $this->_required_method($str, 'delete');
	}
        ...
        if (!empty($this->fields)) {
            foreach ($this->fields as $field) {
                ...
                if ($field instanceof Restserver_rule) {
                    ...
                    if (isset($this->input[$this->method][$field->field])) {
                        $input[$field->field] = & $this->input[$this->method][$field->field];
